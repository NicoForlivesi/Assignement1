...
func runTournament(players []chan PlayRequest) {
    ...
    for len(current) > 1 { // Quando current contiene un solo elemento quello è l'id del vincitore
        ...
        for i := 0; i < len(current)/2; i++ { // Questo è il for che GARANTISCE LA SINCRONIZZAZIONE IMPLICITA
            fmt.Print(<-logs)     // N/2 send su logs (fatte da playMatch)
            next = append(next, <-results) // N/2 send su results (fatte da playMatch)
        }
...
func playMatch(p1, p2 int, players []chan PlayRequest, results chan int, logs chan string) {
    ...
    // Invio ruolo e chiedo numero, specificando per ogni giocatore su che canale voglio la risposta
